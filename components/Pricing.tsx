...
const Pricing: React.FC<PricingProps> = ({ lang }) => {
  ...
  return (
    <section ref={sectionRef} id="pricing" className="py-20 md:py-32 bg-slate-50 dark:bg-slate-950 border-t border-slate-200 dark:border-slate-900/50 relative pb-32 md:pb-32 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        
        {/* HEADER */}
        <div className="text-center mb-10 md:mb-16">
          <span className="text-indigo-600 dark:text-indigo-400/80 font-semibold tracking-[0.2em] uppercase text-[10px] md:text-xs">{t.badge}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 items-start">
          ...
          <div className="lg:col-span-8 space-y-4 md:space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900/20 p-5 md:p-8 rounded-sm border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-none transition-colors">
              <div className="flex items-center gap-2 mb-6">
                <Clock className="text-indigo-600 dark:text-indigo-400 w-4 h-4 md:w-5 md:h-5" />
                <h3 className="text-base md:text-lg font-serif text-slate-900 dark:text-white">{t.coverageTitle}</h3>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 md:gap-10">
                ...
                <div>
                  <div className="flex justify-between text-xs md:text-sm mb-3">
                    <label className="text-slate-600 dark:text-slate-300 font-medium">{t.photographers}</label>
                    <span className="text-indigo-600 dark:text-indigo-400 font-bold">{photographers}</span>
                  </div>
                  <input 
                ...
                <div>
                  <div className="flex justify-between text-xs md:text-sm mb-3">
                    <label className="text-slate-600 dark:text-slate-300 font-medium">{t.duration}</label>
            ...
            <div className="bg-white dark:bg-slate-900/20 p-5 md:p-8 rounded-sm border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-none transition-colors">
              <div className="flex items-center gap-2 mb-6">
                <Camera className="text-indigo-600 dark:text-indigo-400 w-4 h-4 md:w-5 md:h-5" />
                <h3 className="text-base md:text-lg font-serif text-slate-900 dark:text-white">{t.visualTitle}</h3>
              </div>
              <div>
                <div className="flex justify-between text-xs md:text-sm mb-3">
            ...
            <div className="bg-white dark:bg-slate-900/20 p-5 md:p-8 rounded-sm border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-none transition-colors">
              <div className="flex items-center gap-2 mb-6">
                <Globe className="text-indigo-600 dark:text-indigo-400 w-4 h-4 md:w-5 md:h-5" />
                <h3 className="text-base md:text-lg font-serif text-slate-900 dark:text-white">{t.digitalTitle}</h3>
              </div>
                ...
                {[
                    ...
                    <div 
                        ...
                        className={`cursor-pointer p-3 md:p-4 border rounded-sm transition-all relative
                            ${albumType === option.type 
                                ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.15)]' 
                    ...
                    >
                        <div className="flex justify-between items-center md:items-start md:flex-col md:mb-2">
                            <span className={`text-xs md:text-sm font-bold uppercase tracking-wider ${albumType === option.type ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'}`}>
                                {option.label}
                            </span>
                            <span className="text-sm md:text-lg font-light text-slate-900 dark:text-white md:mt-1">{option.price}</span>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
               <div 
                  ...
                  className={`cursor-pointer flex items-center justify-between p-4 md:p-6 rounded-sm border transition-all 
                  ${isRush 
                    ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-500' 
                    : 'bg-white dark:bg-slate-900/20 border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-none'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <Zap className={`w-4 h-4 md:w-5 md:h-5 ${isRush ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-600'}`} />
                    <div>
                      <h4 className="text-xs md:text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">{t.rushTitle}</h4>
                      <p className="text-[10px] md:text-xs text-slate-500">{t.rushDesc}</p>
                    </div>
                  </div>
                  <div className={`w-4 h-4 rounded-full border flex items-center justify-center ${isRush ? 'border-indigo-500 bg-indigo-500' : 'border-slate-300 dark:border-slate-600'}`}>
                    {isRush && <Check className="w-3 h-3 text-white" />}
                  </div>
               </div>

               <div className="bg-white dark:bg-slate-900/20 p-4 md:p-6 rounded-sm border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-none">
                  <div className="flex items-center gap-2 mb-2 md:mb-4">
                    <h4 className="text-xs md:text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">{t.travelTitle}</h4>
                  </div>
                  <select 
                    value={travelZone}
                    onChange={(e) => setTravelZone(e.target.value as TravelZone)}
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-300 text-xs md:text-sm p-2 rounded-sm outline-none focus:border-indigo-500"
                  >
            ...
            <div className="md:hidden mt-6 flex items-start gap-3 p-4 bg-red-50 dark:bg-red-950/10 rounded-sm border border-red-200 dark:border-red-900/20">
                <AlertTriangle className="w-4 h-4 text-red-500 dark:text-red-400/80 mt-0.5 shrink-0" />
                <p className="text-[10px] text-red-600 dark:text-red-200/60 leading-relaxed">{t.disclaimerText}</p>
            </div>
          ...
          <div className="hidden lg:block lg:col-span-4 relative">
            <div className="sticky top-24 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-sm p-8 shadow-xl dark:shadow-2xl">
              ...
              <div className="border-t border-slate-100 dark:border-slate-800 pt-6 mb-8">
                <div className="flex justify-between items-end">
                  ...
                  <span className="text-4xl font-serif text-slate-900 dark:text-white">{formatCurrency(total)}</span>
                </div>
              ...
              <div className="space-y-3">
                <button className="w-full py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-950 font-bold uppercase tracking-widest text-xs hover:bg-indigo-600 dark:hover:bg-indigo-50 transition-colors flex items-center justify-center gap-2">
                  {t.btnBook} <ArrowRight className="w-4 h-4" />
                </button>
                <button className="w-full py-4 border border-slate-300 dark:border-slate-700 text-slate-500 dark:text-slate-300 font-bold uppercase tracking-widest text-xs hover:border-slate-900 dark:hover:border-white hover:text-slate-900 dark:hover:text-white transition-colors flex items-center justify-center gap-2">
      ...
      <div 
        className={`fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 z-50 lg:hidden shadow-[0_-5px_20px_rgba(0,0,0,0.1)] dark:shadow-[0_-5px_20px_rgba(0,0,0,0.5)] transition-transform duration-500 ease-in-out ${isSectionVisible ? 'translate-y-0' : 'translate-y-full'}`}
      >
         {/* Dropdown Details */}
         {showMobileDetails && (
             <div className="bg-white dark:bg-slate-900 p-6 border-b border-slate-200 dark:border-slate-800 animate-in slide-in-from-bottom-5">
         ...
         <div className="p-4 flex items-center justify-between gap-4">
            ...
            <button className="px-6 py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-950 font-bold uppercase tracking-widest text-xs rounded-sm shadow-lg active:scale-95 transition-transform">
                {t.btnBook}
            </button>
         </div>
      </div>
